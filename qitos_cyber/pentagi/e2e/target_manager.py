...
class TargetManager:
    ...
    def __init__(self, target: VulnerableTarget, skip_nmap: bool = False):
        self.target = target
    ...
    def get_ground_truth(self) -> Dict[str, Any]:
        ...
        if self.skip_nmap:
            logger.info("Skipping nmap scan (skip_nmap=True)")
        else:
            try:
    ...
    def _start_container(self) -> None:
        ...
        # Port mappings — use dynamic host ports to avoid conflicts
        self.host_port_map: Dict[int, int] = {}
        for host_port, container_port in self.target.ports.items():
            cmd.extend(["-p", f"0:{container_port}"])
        ...
        # Discover the actual host ports assigned by Docker
        self._discover_host_ports()

    def _discover_host_ports(self) -> None:
        """Discover the actual host ports assigned by Docker."""
        if not self.container_id:
            return
        try:
            ...
            # Parse output like "80/tcp -> 0.0.0.0:49153"
            for line in result.stdout.strip().splitlines():
                parts = line.split(" -> ")
                if len(parts) == 2:
                    container_port_proto = parts[0].strip()
                    container_port = int(container_port_proto.split("/")[0])
                    host_addr = parts[1].strip()
                    host_port = int(host_addr.rsplit(":", 1)[1])
                    self.host_port_map[container_port] = host_port
            logger.info(f"Discovered host ports: {self.host_port_map}")
        except Exception as e:
            logger.warning(f"Failed to discover host ports: {e}")

    def _wait_for_health(self) -> None:
        """Wait for the target to become healthy via HTTP health check."""
        # Use the discovered host port (mapped from container's health_port)
        host_port = self.host_port_map.get(self.target.health_port, self.target.health_port)

...
class TestCollectText:
    ...
    def test_skips_non_string_values(self):
        ...
        _collect_text(result)  # Should not crash
...
class TestPentagiE2EScorer:
    ...
    def test_pipeline_completed_timeout_with_report(self):
        result = FakeResult(status="timeout", report="x" * 300)
        criterion = TierCriterion("p", "", "pipeline_completed")
        passed, detail = self.scorer._check_pipeline_completed(
            criterion, result, self.ground_truth, None
        )
    ...
    def test_pipeline_completed_timeout_no_report(self):
        result = FakeResult(status="timeout", report="short")
        criterion = TierCriterion("p", "", "pipeline_completed")
        passed, detail = self.scorer._check_pipeline_completed(
            criterion, result, self.ground_truth, None
        )

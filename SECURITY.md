...
qitos-zoo is currently in alpha. Security fixes are prioritized for the latest release line on `main`.
...
- avoid hardcoded paths in security tools — use `tempfile` or configurable paths
- never commit datasets, model weights, or binary assets
